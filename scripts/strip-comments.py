#!/usr/bin/env python3
"""Strip inline comments from Python files while preserving docstrings.

This script removes inline comments (lines starting with hash and hash after code)
while preserving docstrings, maintaining code structure, and removing
excessive blank lines.

Usage:
    python scripts/strip-comments.py
        Process all .py files in cli/src/timeblock/
    
    python scripts/strip-comments.py path/to/file.py
        Process single file
...
Transformations applied:
    - Remove lines starting with hash
    - Remove hash inline comments (after code)
    - Preserve docstrings (triple quotes)
    - Maintain blank line between methods/classes
    - Compact code by removing excessive blank lines
"""
import argparse
import sys
...
def is_docstring_delimiter(line: str) -> bool:
    """Check if line contains docstring delimiter."""
    stripped = line.strip()
...
def strip_comments_from_file(content: str) -> str:
    ...
    Returns:
        Processed content without inline comments
    """
    lines = content.split('\n')
    result: List[str] = []
    in_docstring = False
