...
def daily_report(
    ...
    try:
        ...
        total_tracked = sum(
            ...
            for log in timelogs if log.end_time
        )
        ...
        console.print(f"Hábitos: {habits_completed}/{habits_total} completos ({habits_completed/habits_total*100:.0f}%)" if habits_total > 0 else "Hábitos: 0/0")
        console.print(f"Tarefas: {tasks_completed}/{tasks_total} completas ({tasks_completed/tasks_total*100:.0f}%)" if tasks_total > 0 else "Tarefas: 0/0")
        console.print("═" * 50)
        ...
        if instances:
            ...
            for inst in instances:
                ...
                console.print(f"{status} {habit.title} ({inst.scheduled_start.strftime('%H:%M')} → {inst.scheduled_end.strftime('%H:%M')})")
        
        if tasks:
            console.print("\n[bold]Tarefas:[/bold]")
            for task in tasks:
                status = "✓" if task.completed_datetime else "○"
                console.print(f"{status} {task.title} ({task.scheduled_datetime.strftime('%H:%M')})")
...
def weekly_report(
    ...
    try:
        ...
        console.print(f"{start_of_week.strftime('%d/%m/%Y')} a {end_of_week.strftime('%d/%m/%Y')}\n")
        ...
        console.print(f"Hábitos: {week_habits_completed}/{week_habits_total} ({week_habits_completed/week_habits_total*100:.0f}%)" if week_habits_total > 0 else "Hábitos: 0/0")
        console.print(f"Tarefas: {week_tasks_completed}/{week_tasks_total} ({week_tasks_completed/week_tasks_total*100:.0f}%)" if week_tasks_total > 0 else "Tarefas: 0/0")
        console.print("═" * 50)
        console.print()
...
def habit_report(
    ...
    try:
        ...
        if not instances:
            console.print(f"Nenhuma instância encontrada para [bold]{habit.title}[/bold] nos últimos {days} dias", style="yellow")
            return
        ...
        for inst in recent:
            status = "✓" if inst.actual_end else "✗"
            console.print(f"{status} {inst.date.strftime('%d/%m/%Y')} - {inst.scheduled_start.strftime('%H:%M')} → {inst.scheduled_end.strftime('%H:%M')}")
    ...
    try:
        ...
        while current <= end_date:
            ...
            tasks = TaskService.list_tasks(
                ...
                end_datetime=current + timedelta(days=1)
            )
            ...
            if instances or tasks:
                ...
                for inst in instances:
                    habit = HabitService.get_habit(inst.habit_id)
                    console.print(f"  {inst.scheduled_start.strftime('%H:%M')} → {inst.scheduled_end.strftime('%H:%M')} | {habit.title}")
                
                for task in tasks:
                    console.print(f"  {task.scheduled_datetime.strftime('%H:%M')} | {task.title} (tarefa)")
