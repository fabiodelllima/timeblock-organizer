...
- Planned time preservation in `HabitInstance` — `original_scheduled_start` and `original_scheduled_end` hold the planned schedule while `scheduled_start`/`scheduled_end` now mean the effective one, so the divergence between plan and execution survives a `habit adjust` instead of being destroyed at the moment it is recorded. `generate_instances` fixes the planned values from the Habit template; `adjust_instance_time` captures the current values when they are absent, before overwriting, and increments `adjustment_count` only when the schedule actually changes. Schema migration `migration_005_habitinstance_planned_time` (idempotent `ALTER TABLE ADD COLUMN` via `PRAGMA table_info`, with backfill from the current schedule). Prerequisite for the duration analytics planned in #49, which computes planned duration from those columns. Implements BR-HABITINSTANCE-008 (new), following the BR-TASK-008 precedent. (Closes #70)
